...
ensure_repo() {
  ...
  if git -C "${HIST_DIR}" remote | grep -q '^origin$'; then
    LOG "尝试从远程仓库拉取数据（浅克隆，深度=1）..."
    git -C "${HIST_DIR}" fetch --depth=1 origin main || true
    git_sanitize_repo
    git -C "${HIST_DIR}" pull --depth=1 --rebase --autostash origin main || true
  fi
}
...
commit_and_push() {
  ...
  if git -C "$dir" remote | grep -q '^origin$'; then
    ...
    for attempt in 1 2 3; do
      ...
      if ! git -C "$dir" pull --depth=1 --rebase --autostash origin main; then
        git -C "$dir" rebase --abort >/dev/null 2>&1 || true
...
update() {
  if git -C "${HIST_DIR}" remote | grep -q '^origin$'; then git -C "${HIST_DIR}" pull --depth=1 --rebase --autostash origin main || true; fi
  link_targets
  pointerize_large_files
  wait_until_hydrated
  commit_and_push
